    ...
    switch (btn.tag) {
        case 0:
        {
            if (_intentArr.count) {
                ...
                [BaseRequest GET:ProjectBusinessGetBasicsList_URL parameters:@{@"project_id":self.allDic[@"project_id"],@"type":@"1"} success:^(id  _Nonnull resposeObject) {
...
- (void)initUI{
    ...
    NSArray *titleArr = @[@"合作意向：",@"跟进方式：",@"跟进阶段：",@"跟进时间：",@"跟进内容："];
    ...
    for (int i = 0; i < titleArr.count; i++) {
        ...
        switch (i) {
            ...
            case 3:
            {
                ...
                _timeBtn = btn;
                _timeBtn.content.text = [_formatter stringFromDate:[NSDate date]];
...
- (void)MasonryUI{
    ...
    [_wayL mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self->_scrollView).offset(9 *SIZE);
        make.top.equalTo(self->_intentBtn.mas_bottom).offset(26 *SIZE);
        make.width.mas_equalTo(70 *SIZE);
    }];
    ...
    [_stageL mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self->_scrollView).offset(9 *SIZE);
        make.top.equalTo(self->_wayBtn.mas_bottom).offset(26 *SIZE);
        make.width.mas_equalTo(70 *SIZE);
    }];
    ...
    [_timeL mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self->_scrollView).offset(9 *SIZE);
        make.top.equalTo(self->_stageBtn.mas_bottom).offset(26 *SIZE);
        make.width.mas_equalTo(70 *SIZE);
    }];
