...
@interface AddIntentStoreProccessCell ()<UICollectionViewDelegateFlowLayout,UICollectionViewDelegate,UICollectionViewDataSource>
{
...
@implementation AddIntentStoreProccessCell
...
- (void)ActionTypeBtn:(UIButton *)btn{
    ...
    if (self.addIntentStoreProccessCellTypeBlock) {
        
        self.addIntentStoreProccessCellTypeBlock();
    }
}

- (void)ActionAuditBtn:(UIButton *)btn{
    
    if (self.addIntentStoreProccessCellAuditBlock) {
        
        self.addIntentStoreProccessCellAuditBlock();
    }
}

- (void)ActionRoleBtn:(UIButton *)btn{
    
    if (self.addIntentStoreProccessCellRoleBlock) {
        
        self.addIntentStoreProccessCellRoleBlock();
    }
}

- (void)ActionFinalBtn:(UIButton *)btn{
    
    if (self.addIntentStoreProccessCellFinalBlock) {
        
        self.addIntentStoreProccessCellFinalBlock();
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.addIntentStoreProccessCellSelectBlock) {
        
        self.addIntentStoreProccessCellSelectBlock(_selectArr);
    }
...
- (void)initUI{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (i == 1) {
            ...
            _roleBtn.hidden = YES;
            [self.contentView addSubview:_roleBtn];
        }else if(i == 3){
            
            _personL = label;
            _personL.hidden = YES;
            [self.contentView addSubview:_personL];
            ...
            _finalL = label;
            _finalL.hidden = YES;
            [self.contentView addSubview:_finalL];
            ...
            [self.contentView addSubview:_finalBtn];
        }
    }
