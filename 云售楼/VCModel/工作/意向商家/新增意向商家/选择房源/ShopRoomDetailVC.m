...
@interface ShopRoomDetailVC ()<SMCinameSeatScrollViewDelegate,UIScrollViewDelegate>
...
@implementation ShopRoomDetailVC
...
-(void)initInterFace
{
    ...
    for (int i = 0; i<_row; i++) {
        ...
        if ([_LDinfo[i][@"shopList"] count] >_column) {
            _column = [_LDinfo[i][@"shopList"] count];
        }
    }
...
//    NSLog(@"_myScrollView.zoomScale = %f",_myScrollView.zoomScale);
    ...
    if (!self.rowIndexView) {
        ...
        for (NSUInteger i = 0; i<_LDinfo.count; i++) {
            [self.rowIndexView.arrayRowIndex addObject:[NSString stringWithFormat:@"%@",_LDinfo[i][@"floor_num"]]];
        }
...
- (void)drawSeat{
    ...
    for (NSUInteger row = 0; row < self.row; row++) {
        ...
        for (NSUInteger column = 0; column < [_LDinfo[row][@"shopList"] count]; column++) {
            ...
            if (arraySeat.count <= column) {
                ...
                [btnSeat setTitle:_LDinfo[row][@"shopList"][column][@"name"] forState:UIControlStateNormal];
...
//                {
                if ([_LDinfo[row][@"shopList"][column][@"state"] integerValue]==0||[_LDinfo[row][@"shopList"][column][@"state"] integerValue]==1) {
                    btnSeat.backgroundColor = COLOR(0, 197, 198, 1);//KweixuanColor;
                }
                
//
//                }
                else if ([_LDinfo[row][@"shopList"][column][@"state"] integerValue] ==4)
                {
...
- (void)btnSeatTouchIn:(UIButton *)btn {
    ...
    [BaseRequest GET:ShopGetShopDetail_URL parameters:@{@"shop_list":_LDinfo[btn.tag][@"shopList"][columns][@"shop_id"]} success:^(id  _Nonnull resposeObject) {
