//
//  StoreOrderMinPriceProcessCell.m
//  云售楼
//
...
@implementation StoreOrderMinPriceProcessCell
...
- (void)ActionAuditBtn:(UIButton *)btn{
    ...
    if (self.storeOrderMinPriceProcessCellAuditBlock) {

        self.storeOrderMinPriceProcessCellAuditBlock(self.tag);
    }
}
...
- (void)setDataDic:(NSMutableDictionary *)dataDic{
    ...
    _intentNumL.text = [NSString stringWithFormat:@"定租铺号：%@",dataDic[@"row_code"]];
    ...
    _intentCodeL.text = [NSString stringWithFormat:@"定租编号：%@",dataDic[@"row_code"]];
    _UnitMinPriceL.text = [NSString stringWithFormat:@"租金单价底价：%@",dataDic[@"row_code"]];
...
- (void)initUI{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        switch (i) {
            ...
            case 3:
            {
                _firstUnitPriceL = label;
                [_whiteView addSubview:_firstUnitPriceL];
                break;
            }
            case 4:
            {
            ...
            case 7:
            {
                _UnitMinPriceL = label;
                [_whiteView addSubview:_UnitMinPriceL];
                break;
            }
            case 8:
            {
...
- (void)masonryUI{
    ...
    [_firstUnitPriceL mas_makeConstraints:^(MASConstraintMaker *make) {
...
//        make.bottom.equalTo(self->_whiteView).offset(-18 *SIZE);
    ...
    [_UnitMinPriceL mas_makeConstraints:^(MASConstraintMaker *make) {
