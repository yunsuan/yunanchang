...
#import "ShopChangeNumAuditCell.h"

@implementation ShopChangeNumAuditCell
...
- (void)ActionAuditBtn:(UIButton *)btn{
    ...
    if (self.shopChangeNumAuditCellAuditBlock) {

        self.shopChangeNumAuditCellAuditBlock(self.tag);
    }
}
...
- (void)setDataDic:(NSMutableDictionary *)dataDic{
    ...
    _shopNameL.text = [NSString stringWithFormat:@"商家名称：%@",dataDic[@"business_name"]];
    ...
    _changeCodeL.text = [NSString stringWithFormat:@"变更铺号：%@",dataDic[@"sign_agent_name"]];
}
...
- (void)initUI{
    ...
    for (int i = 0; i < 7; i++) {
        ...
        switch (i) {
            ...
            {
                _changeCodeL = label;
                [_whiteView addSubview:_changeCodeL];
                break;
            }
            default:
                break;
...
- (void)masonryUI{
    ...
    [_changeCodeL mas_makeConstraints:^(MASConstraintMaker *make) {
