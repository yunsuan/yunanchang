...
@interface ShopAuditTaskDetailVC ()<UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,UITextViewDelegate>
{
...
@implementation ShopAuditTaskDetailVC
...
- (void)RequestMethod{
    ...
    [BaseRequest GET:TradeGetProgreesList_URL parameters:@{@"type":self.status,@"id":self.requestId} success:^(id  _Nonnull resposeObject) {
...
- (void)ActionAuditBtn:(UIButton *)btn{
    ...
    [BaseRequest POST:TradeProgressPass_URL parameters:dic success:^(id  _Nonnull resposeObject) {
        
        if ([resposeObject[@"code"] integerValue] == 200) {
            
            [self showContent:@"审核成功"];
            if (self.shopAuditTaskDetailVCBlock) {
                
                self.shopAuditTaskDetailVCBlock();
            }
...
- (void)ActionDisagreeBtn:(UIButton *)btn{
    ...
    [dic setValue:[NSString stringWithFormat:@"%ld",[self.status integerValue] - 1] forKey:@"from_type"];
    
    [BaseRequest POST:TradeProgressRefuse_URL parameters:dic success:^(id  _Nonnull resposeObject) {
