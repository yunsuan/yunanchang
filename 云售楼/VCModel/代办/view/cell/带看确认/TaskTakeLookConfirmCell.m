...
@implementation TaskTakeLookConfirmCell
...
- (void)initUI{
    ...
    _copBtn.layer.cornerRadius = 2 *SIZE;
    _copBtn.layer.borderWidth = SIZE;
    _copBtn.layer.borderColor = CLBlueBtnColor.CGColor;
    _copBtn.clipsToBounds = YES;
    _copBtn.titleLabel.font = [UIFont systemFontOfSize:13 *SIZE];
    [_copBtn addTarget:self action:@selector(ActionCopyBtn:) forControlEvents:UIControlEventTouchUpInside];
    [_copBtn setTitle:@"复制" forState:UIControlStateNormal];
    [_copBtn setTitleColor:CLBlueBtnColor forState:UIControlStateNormal];
    [_whiteView addSubview:_copBtn];
...
- (void)masonryUI{
    ...
    [_copBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self->_whiteView.mas_right).offset(-77 *SIZE);
